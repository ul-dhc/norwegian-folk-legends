...
export type Legend={id:string;volume:string;chapter:Localized;title:Localized;text:Localized;collectorId:string|null;narratorId:string|null;placeId:string|null;countyId:string|null;mlCategoryId:string|null;genre:string|null;subgenre:string|null;year:string;originalMetadata:string|null;comments:string|null;notes:string|null;originalCoordinates:{latitude:number;longitude:number}|null;primarySourceRaw:string|null;secondarySourceRaw:string|null;archiveSignature:string|null;textSignature:string|null;sourceUrl:string|null};
...
export const browseRecords=legends.map(legend=>{const data=resolveLegend(legend);return{id:legend.id,volume:legend.subgenre||'',chapterNo:legend.chapter.no||'',chapterEn:legend.chapter.en||'',placeId:legend.placeId||'',titleNo:legend.title.no||data.category?.title||legend.id,titleEn:legend.title.en||'',place:data.place?.name||'',collector:displayPersonName(data.collector),narrator:displayPersonName(data.narrator),year:legend.year,excerptNo:excerpt(legend.text.no),excerptEn:excerpt(legend.text.en)}});
